...
def build_queue_callbacks(queue_worker):
    ...
    # This is just a helper - should be stored in a proper config enum/obj similar what was proposed in
    ...
    # similarly we should have structure for status updates for now defining only here:
    status = {"perentual_status": '80', 'status': 'ATTRIBUTEEXTRACTION_STARTED'}
    ...
    queue_worker.add_listener(classificationComplete.QUEUE_NAME, partial(on_generic_complete_event, status=status, dest_queue_name=destionation_queue_name))
...
def on_triage_complete(msg_proc_obj, message_publisher):
    ...
    if msg_proc_obj['OCR_Required'] == True:
        ...
        status = {"perentual_status": '30', 'status': 'OCR_STARTED'}
    else:
        dest_queue = 'classification_request'
        status = {"perentual_status": '70', 'status': 'CLASSIFICATION_STARTED'}
