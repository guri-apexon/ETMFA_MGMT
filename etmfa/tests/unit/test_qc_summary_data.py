...
#         all_summary_records = utils.get_summary_records(
#             aidocid, source=summary_config.SRC_QC)
...
#                           ("SSR_1002-043", "  larger text than target length", "larger t", "",
#                           datetime.date(year=1900, month=1, day=1), "Text truncation and empty date"),
#                           ("SSR_1002-043", "larger text than target length", "larger t", "20210606",
#                           datetime.date(year=2021, month=6, day=6), "Valid approval date1"),
#                           ("SSR_1002-043", "larger text than target length", "larger t", " 20210607  ",
#                           datetime.date(year=2021, month=6, day=7), "Valid approval date2"),
#                           ("SSR_1002-043", "larger text than target length", "larger t", " A0210608  ",
#                           datetime.date(year=1900, month=1, day=1), "Invalid date check")
#                           ])
...
#         # Clear old test data
#         test_qc_summary = PDProtocolQCSummaryData.query.filter(
#             PDProtocolQCSummaryData.aidocId == aidoc_id, PDProtocolQCSummaryData.source == summary_config.SRC_EXTRACT, PDProtocolQCSummaryData.protocolNumber == protocol_num).delete()
#         test_protocol_data = Protocoldata.query.filter(
#             Protocoldata.id == aidoc_id).delete()
#         test_protocol_qcdata = Protocolqcdata.query.filter(
#             Protocolqcdata.id == aidoc_id).delete()
#         logging.info(
#             f"Unit test {comments}: Cleared # of rows of [qc_summary: {test_qc_summary}]; [protocol_data: {test_protocol_data}]; [protocol_qcdata: {test_protocol_qcdata}]")
#         db_context.session.commit()

#         # Setup test data
#         summary_json_dict = ast.literal_eval(finalattributes_dict['summary'])
#         summary_json_dict_data_updated = [[name, assign_is_amendment_flg, alias] if name == "is_amendment" else [
#             name, value, alias] for name, value, alias in summary_json_dict['data']]
#         summary_json_dict_data_updated = [[name, assign_approval_date, alias] if name == "approval_date" else [
#             name, value, alias] for name, value, alias in summary_json_dict_data_updated]
#         summary_json_dict['data'] = summary_json_dict_data_updated
#         finalattributes_dict['summary'] = str(json.dumps(summary_json_dict))

#         # Run test
#         _ = received_finalizationcomplete_event(id=aidoc_id, finalattributes={
#                                                 'db_data': finalattributes_dict}, message_publisher=None)
#         all_summary_records = utils.get_summary_records(
#             aidoc_id, source=summary_config.SRC_EXTRACT)
