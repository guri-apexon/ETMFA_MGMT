...
@pytest.mark.parametrize("protocol_number, version_number, approval_date, id, document_status, response_status, comments", 
...
def test_dynamic_rules(new_app_context, protocol_number, version_number, approval_date, id, document_status, response_status, comments):
    new_app, _ = new_app_context
    client = new_app.test_client()
    input_dict = {"protocolNumber": protocol_number, "versionNumber":version_number, "approvalDate": approval_date, "id":id, "documentStatus": document_status}
    with client:
        logger.debug(
            f"test_get_protocols: Processing for unit test type [{comments}]: [{document_status}, {id}, {approval_date}]")
