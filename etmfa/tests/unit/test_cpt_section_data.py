...
@pytest.mark.parametrize("doc_id, link_level, link_id, user_id, protocol, toc, section_text, config_variables, status_code, comments", [
    ("00239212-1359-4d00-b7b1-24ff7fbeea7b", '3', "6f7f6126-be2f-11ed-8105-005056ab6469", "Dig2_Batch_Tester",
     "cicl.06ed2096-0749-4ead-a892-9e57ead4fcbc","","","", 200, "doc id and with link id and link_level 3"),
    ("1698be28-1cf3-466e-8f56-5fc920029056", "1", "", "1036048",
     "FEED_TEST4","","","", 404, "doc id changes"),
...
def test_configurable_params(new_app_context, user_id, protocol, doc_id, status_code, link_level, link_id, toc, section_text, config_variables, comments):
    ...
    client = new_app.test_client()
    with client:
        get_cpt_section_data = client.get("/pd/api/cpt_data/get_section_data_configurable_parameter",
                                          json={"aidoc_id": doc_id,
                                                "link_level": link_level,
                                                "link_id": link_id,
                                                "user_id": user_id,
